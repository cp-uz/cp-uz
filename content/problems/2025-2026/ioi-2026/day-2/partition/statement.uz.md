...
Temur va uning yordamchisi Ulug'bek _Uzbekiston Got Talent_ ko'rsatuvi uchun sehrli fokus tayyorlamoqda. Fokus Temurning quyidagi **guruhlash muammosi** ni hal qilishiga asoslanadi: berilgan musbat butun sonlar to'plamini $K$ ta bo'sh bo'lmagan guruhlarga shunday bo'lish kerakki, har bir guruhdagi elementlar yig'indisi bir xil bo'lsin. Boshqacha qilib aytganda, to'plamdagi har bir butun son $K$ guruhning aniq bittasiga biriktirilishi va har bir guruhdagi yig'indi teng bo'lishi kerak. Masalan, agar berilgan to'plam [2,1,6,4,5] va $K = 3$ bo'lsa, to'g'ri bo'linish [2,4], [1,5] va [6] bo'ladi. Bu holda har bir guruhdagi elementlar yig'indisi aniq 6 ga teng bo'ladi.

Fokus quyidagicha bajariladi:

- Ulug'bek va Temur alohida kabinalarga kiradilar va bir-birlari bilan muloqot qila olmaydilar. Hakamlar Ulug'bekka $N$ musbat butun sonlardan iborat $A$ massivini beradi, $A[0]$, $ A[1]$,… , $ A $ [ $ N - 1$] , bu yerda har bir qiymat 1 va $ M$ oralig'ida bo'ladi. Hakamlar Ulug'bekka $ K$ qiymatini ham beradi.
...
Sizning vazifangiz Temur va Ulug'bek uchun strategiya ishlab chiqish va uni amalga oshirish. Strategiya doimo mavjud. Ya'ni, berilgan cheklovlar ostida, hakamlar tomonidan taqdim etilgan $A$ massividan qat'i nazar, ular uchun bo'lish muammosini muvaffaqiyatli hal qilish imkonini beruvchi strategiya mavjudligini isbotlash mumkin.
...
- $M$ : har bir asl va yangi qo'shilgan butun son uchun ruxsat etilgan maksimal qiymat. Ushbu protsedura har bir test uchun aniq bir marta chaqiriladi.
...
- $B $ : uzunligi $ N $ + $ S $ bo'lgan massiv bo'lib, unda $ A $ dagi asl butun sonlar va Ulug'bek qo'shgan butun sonlar mavjud. $ B $ massivining elementlari kamaymaydigan tartibda saralangan. $ K$ : bo'linishi kerak bo'lgan guruhlar soni.
...
- Har bir $j$ ($0 \le j < N $ + $ S $ ) uchun, $ P[j]$ bu $ B[j]$ tegishli bo'lgan guruhni bildiradi.

- Guruhlar 0 dan $K - 1$ gacha raqamlanishi kerak, ya'ni har bir $0 \le j < N$ + $ S $ uchun $0 \le P[j] < K$ bo'lishi kerak.

- 0 va $K - 1$ oralig'idagi har bir $i$ uchun, $P[j] = i $ bo'lgan kamida bitta $j $ ( $0 \le j < N $ + $ S$ ) elementi bo'lishi kerak.
...
Ushbu misolda $A$ = [8,2,9,6,1,5,5] . Biz raqamlarni $ K = 3$ ta guruhga bo'lishni xohlaymiz. Ulugʻbek 1 va $ M = 9$ oraligʻidagi sonlarni qoʻsha oladi.
...
Ushbu massivdagi butun sonlarni quyidagi uchta guruhga bo'lishimiz mumkin: [1,5,9] , [2,5,8] va [4,5,6] . E'tibor bering, ushbu guruhlarning har biri yig'indisi 15 ga teng. Ushbu guruhlashni berish uchun, protsedura [0,1,2,0,1,2,2,1,0] massivini qaytarishi kerak.
