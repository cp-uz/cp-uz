...
export default function ArticlePage() {
  ...
  const { data: article, loading: articleLoading, error: articleLoadError } = useAsyncData(
    () => learningApi.getArticle(slug, category),
    null,
    [category, slug]
  );
  ...
  const tocItems = useMemo(() => {
    ...
    return practices.length ? [...headings, { id: 'mashqlar', label: 'Mashq qilish', level: 2 }] : headings;
  ...
  useEffect(() => {
    ...
    if (!getAuthSession() || !progressHydrated || hydratedArticleKey !== articleKey) return;
  ...
  useEffect(() => {
    let active = true;
    const localNote = localStorage.getItem(`cpuz:note:${articleKey}`)
    ...
    if (getAuthSession()) {
      engagementApi.listNotes().then((entries) => {
        if (!active) return;
  ...
  if (!article) {
    return (
      <Container maxWidth="sm" sx={{ py: { xs: 8, md: 12 }, textAlign: 'center' }}>
        <UiIcon icon={articleLoadError ? 'solar:server-square-cloud-linear' : 'solar:document-text-linear'} width={44} />
  ...
  const reviewLabel = article.editorialStatus === 'published'
    ? 'Nashr qilingan'
  ...
  const isArticleCompleted = progressReady && (
    readingProgress >= 100 || completed.has(articleKey)
  );
  const related = allArticles
    .filter(
      (item) =>
        (item.sourceId ?? item.slug) !== (article.sourceId ?? article.slug) &&
        (item.categoryId === article.categoryId || item.tags.some((tag) => article.tags.includes(tag)))
    )
    .slice(0, 4);
  ...
  const requestShare = async () => {
    const prefersNativeShare = canNativeShare && (
    ...
    <>
      ...
      {!!article.content && (
        <LinearProgress
          ...
          sx={{ position: 'fixed', zIndex: (theme) => theme.zIndex.appBar + 1, top: 0, left: 0, right: 0, height: 3 }}
        />
      )}

      <Container maxWidth="lg" sx={{ pt: { xs: 4, md: 6 }, pb: { xs: 4, md: 5 } }}>
        <Breadcrumbs separator="/" aria-label="Sahifa yo‘li" sx={{ mb: 3 }}>
          <Link component={RouterLink} to="/" underline="hover" color="text.secondary">Bosh sahifa</Link>
          <Link component={RouterLink} to="/algoritmlar" underline="hover" color="text.secondary">Algoritmlar</Link>
          <Typography variant="body2" color="text.primary">{article.category}</Typography>
        </Breadcrumbs>

        <Box sx={{ maxWidth: 900 }}>
          <Stack direction="row" spacing={1.25} alignItems="center" sx={{ flexWrap: 'wrap' }}>
            <Typography variant="subtitle2" sx={{ color: 'primary.main' }}>{article.category}</Typography>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>·</Typography>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>{article.difficulty}</Typography>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>·</Typography>
            <Stack direction="row" spacing={0.75} alignItems="center">
              <Box
                sx={{
                  ...
                  height: 7,
                  borderRadius: '50%',
                  bgcolor: article.editorialStatus === 'published'
                    ? 'success.main'
                    : article.editorialStatus === 'draft'
                      ? 'warning.main'
                      : 'primary.main',
          ...
          <Typography component="h1" variant="h3" sx={{ mt: 2 }}>{article.title}</Typography>
          <Typography variant="h6" sx={{ mt: 1.5, maxWidth: 820, color: 'text.secondary', fontWeight: 'fontWeightRegular' }}>
            {article.summary}
          </Typography>

          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 1, sm: 2.5 }} alignItems={{ sm: 'center' }} sx={{ mt: 3 }}>
            <Stack direction="row" spacing={0.75} alignItems="center" sx={{ color: 'text.secondary' }}>
              <UiIcon icon="solar:users-group-rounded-linear" width={18} />
              <Typography variant="body2" sx={{ color: 'inherit' }}>
                {contributors.length ? contributors.map((item) => item.name).join(', ') : 'cp.uz tarjima jamoasi'}
              </Typography>
            </Stack>
            <Stack direction="row" spacing={0.75} alignItems="center" sx={{ color: 'text.secondary' }}>
              <UiIcon icon="solar:clock-circle-linear" width={18} />
              <Typography variant="body2" sx={{ color: 'inherit' }}>{article.readTime} daqiqada o‘qiladi</Typography>
            </Stack>
            {article.updatedAt && (
              <Stack direction="row" spacing={0.75} alignItems="center" sx={{ color: 'text.secondary' }}>
          ...
          <Stack direction="row" spacing={1} sx={{ mt: 3, flexWrap: 'wrap' }}>
            <Button
              ...
              startIcon={<UiIcon icon={bookmarks.has(articleKey) ? 'solar:bookmark-bold' : 'solar:bookmark-linear'} width={18} />}
            >
            ...
            <Button
              ...
              startIcon={<UiIcon icon={note.trim() ? 'solar:notes-bold' : 'solar:notes-linear'} width={18} />}
            >
              {note.trim() ? 'Qayd bor' : 'Qayd'}
            </Button>
            <Button color="inherit" onClick={() => void requestShare()} startIcon={<UiIcon icon="solar:share-linear" width={18} />}>Ulashish</Button>
          </Stack>

          {!!article.prerequisites.length && (
            <Stack direction="row" spacing={1} sx={{ mt: 3, flexWrap: 'wrap' }}>
              <Typography variant="body2" sx={{ color: 'text.secondary' }}>Oldindan bilish foydali:</Typography>
              {article.prerequisites.map((item) => (
                <Link key={item} component={RouterLink} to={`/algoritmlar?q=${encodeURIComponent(item)}`} variant="body2" underline="hover">{item}</Link>
              ))}
            </Stack>
          )}
        </Box>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
        <Box sx={{ gap: { md: 4, lg: 6 }, display: 'grid', alignItems: 'start', gridTemplateColumns: { xs: 'minmax(0, 1fr)', md: '190px minmax(0, 720px) 48px' }, justifyContent: 'center' }}>
          <Box
            component="aside"
            sx={{
              position: 'sticky',
          ...
          >
                ...
                {tocItems.map((item) => (
                  <Box
                    ...
                    sx={{
                      ...
                      typography: 'body2',
                      fontWeight: activeSection === item.id ? 'fontWeightSemiBold' : 'fontWeightRegular',
                      textDecoration: 'none',
                    }}
                  >
                    {item.label}
              ...
              {progressReady ? (
                <>
                  <Typography variant="caption" sx={{ color: 'text.secondary' }}>{Math.round(readingProgress)}% o‘qildi</Typography>
                  <LinearProgress variant="determinate" value={readingProgress} sx={{ mt: 1, height: 4 }} />
                </>
              ) : (
                <Typography variant="caption" sx={{ color: 'text.secondary' }}>O‘qish holati tiklanmoqda…</Typography>
              )}
          </Box>

          <Box component="article" id="reader-content" sx={{ minWidth: 0 }}>
            <Accordion disableGutters elevation={0} sx={{ mb: 3, display: { md: 'none' }, bgcolor: 'background.neutral', '&::before': { display: 'none' } }}>
              <AccordionSummary expandIcon={<UiIcon icon="solar:alt-arrow-down-linear" width={18} />}>
                <Typography variant="subtitle2">Mundarija</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Stack>
                  {tocItems.map((item) => <Link key={item.id} href={`#${item.id}`} underline="hover" sx={{ py: 0.75 }}>{item.label}</Link>)}
                </Stack>
              </AccordionDetails>
            </Accordion>
            ...
            <Box
              ...
              sx={{
                '& .rich-markdown': { color: 'text.primary', fontFamily: 'inherit', fontSize: '1rem', lineHeight: 1.8 },
                '& .rich-markdown h2': { mt: 6, mb: 2, fontFamily: 'inherit', fontSize: { xs: '1.75rem', md: '2rem' }, lineHeight: 1.35, letterSpacing: 0 },
                '& .rich-markdown h3': { mt: 4, mb: 1.5, fontFamily: 'inherit', fontSize: '1.35rem', lineHeight: 1.45, letterSpacing: 0 },
                '& .rich-markdown p': { mb: 2.5, color: 'text.primary' },
                '& .rich-markdown blockquote': { my: 3, mx: 0, py: 1, pl: 2.5, borderLeft: 3, borderColor: 'primary.main', bgcolor: 'transparent' },
                '& .rich-markdown pre, & .rich-markdown code': { fontFamily: 'JetBrains Mono Variable, monospace' },
            ...
            {isBinaryFallback && (
              <Box component="section" sx={{ mt: 6 }}>
                ...
                <Typography variant="body2" sx={{ mt: 1, mb: 2.5, color: 'text.secondary' }}>Tilni almashtirib bir xil invariant qanday ifodalanishini solishtiring.</Typography>
                <CodeBlock samples={[{ language: 'cpp', label: 'C++', code: cppCode }, { language: 'python', label: 'Python', code: pythonCode }]} title="first_true" />
              </Box>
            )}
            ...
            {!!practices.length && (
              <Box component="section" id="mashqlar" sx={{ mt: 7, scrollMarginTop: 1 }}>
                ...
                <Typography variant="body2" sx={{ mt: 1, color: 'text.secondary' }}>Havolalar asl platformada ochiladi; masala sharti bu yerga ko‘chirilmaydi.</Typography>
                <Stack divider={<Divider flexItem />} sx={{ mt: 2.5 }}>
                  {practices.map((problem, index) => (
                    <Box
                      ...
                      sx={{ py: 2, gap: 2, display: 'flex', alignItems: 'center', color: 'text.primary', textDecoration: 'none', '&:hover strong': { color: 'primary.main' } }}
                    >
                      <Typography variant="caption" sx={{ width: 26, color: 'text.secondary', fontFamily: 'JetBrains Mono Variable' }}>{String(index + 1).padStart(2, '0')}</Typography>
                      <Box sx={{ minWidth: 0, flexGrow: 1 }}>
                        <Typography component="strong" variant="subtitle2">{problem.title}</Typography>
                        <Typography variant="caption" sx={{ mt: 0.25, display: 'block', color: 'text.secondary' }}>{problem.platform} · {problem.difficulty}{problem.note ? ` · ${problem.note}` : ''}</Typography>
                      </Box>
            ...
            <Box sx={{ mt: 6, p: 2.5, bgcolor: 'background.neutral', borderRadius: 1.5 }}>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }} justifyContent="space-between">
                <Box>
                  <Typography variant="subtitle1">{isArticleCompleted ? 'Darslik yakunlandi' : 'Mavzuni o‘qib bo‘ldingizmi?'}</Typography>
                  <Typography variant="body2" sx={{ mt: 0.25, color: 'text.secondary' }}>
                    {getAuthSession() ? 'Holat profilingizda saqlanadi.' : 'Holat shu brauzerda saqlanadi.'}
                  </Typography>
                </Box>
                {isArticleCompleted ? (
                  <Stack direction="row" spacing={0.75} alignItems="center" sx={{ color: 'success.dark' }}>
                    <UiIcon icon="solar:check-circle-linear" width={20} />
                    <Typography variant="subtitle2">Yakunlangan</Typography>
                  </Stack>
                ) : (
                  <Button color="primary" variant="contained" onClick={() => setReadingProgress(100)}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="space-between">
              <Box>
                <Typography variant="subtitle2">{contributors.length ? `${contributors.length} hissa qo‘shuvchi` : 'Hamjamiyat tayyorlagan maqola'}</Typography>
            ...
            {(article.previous || article.next) && (
              <Box component="nav" aria-label="Qo‘shni darsliklar" sx={{ mt: 5, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' } }}>
                {article.previous ? (
                  <Box component={RouterLink} to={getArticlePath(article.previous)} sx={{ py: 2.5, pr: 2.5, color: 'text.primary', textDecoration: 'none' }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>← Oldingi</Typography>
                    <Typography variant="subtitle1" sx={{ mt: 0.5 }}>{article.previous.title}</Typography>
                  </Box>
                ) : <span />}
                {article.next && (
                  <Box component={RouterLink} to={getArticlePath(article.next)} sx={{ py: 2.5, pl: { sm: 2.5 }, borderLeft: { sm: 1 }, borderColor: 'divider', color: 'text.primary', textAlign: { sm: 'right' }, textDecoration: 'none' }}>
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>Keyingi →</Typography>
                    <Typography variant="subtitle1" sx={{ mt: 0.5 }}>{article.next.title}</Typography>
                  </Box>
                )}
              </Box>
            )}
          ...
          <Box component="aside" sx={{ position: 'sticky', top: 88, alignSelf: 'start', display: { xs: 'none', md: 'block' } }}>
            <Stack spacing={0.75}>
              <Tooltip title={bookmarks.has(articleKey) ? 'Saqlangan' : 'Saqlash'} placement="left">
                <IconButton color={bookmarks.has(articleKey) ? 'primary' : 'default'} onClick={() => bookmarks.toggle(articleKey)}><UiIcon icon={bookmarks.has(articleKey) ? 'solar:bookmark-bold' : 'solar:bookmark-linear'} /></IconButton>
              </Tooltip>
              <Tooltip title={note.trim() ? 'Shaxsiy qayd saqlangan' : 'Shaxsiy qayd'} placement="left">
                <IconButton color={note.trim() ? 'primary' : 'default'} onClick={() => setNotesOpen(true)}>
                  <UiIcon icon={note.trim() ? 'solar:notes-bold' : 'solar:notes-linear'} />
                </IconButton>
              </Tooltip>
              <Tooltip title="Ulashish" placement="left"><IconButton onClick={() => void requestShare()}><UiIcon icon="solar:share-linear" /></IconButton></Tooltip>
            </Stack>
          </Box>
        </Box>
      </Container>
      ...
      {!!related.length && (
        <Box component="section" sx={{ py: { xs: 6, md: 8 }, bgcolor: 'background.neutral' }}>
          <Container maxWidth="md">
            ...
            <Stack divider={<Divider flexItem />} sx={{ mt: 2.5 }}>
              {related.map((item) => (
                <Box key={item.sourceId ?? item.slug} component={RouterLink} to={getArticlePath(item)} sx={{ py: 2.25, gap: 2, display: 'flex', alignItems: 'center', color: 'text.primary', textDecoration: 'none' }}>
                  <Box sx={{ flexGrow: 1 }}><Typography variant="subtitle1">{item.title}</Typography><Typography variant="body2" sx={{ mt: 0.5, color: 'text.secondary' }}>{item.category} · {item.readTime} daqiqa</Typography></Box>
      ...
      >
        <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={2} sx={{ p: 3 }}>
          <Box sx={{ minWidth: 0 }}>
            <Typography variant="subtitle2" sx={{ color: 'primary.main' }}>Shaxsiy qayd</Typography>
            <Typography variant="h5" sx={{ mt: 0.75 }}>{article.title}</Typography>
          </Box>
          <IconButton aria-label="Qaydni yopish" onClick={() => setNotesOpen(false)}><UiIcon icon="solar:close-circle-linear" /></IconButton>
        </Stack>
        <Divider />
        <Box sx={{ p: 3, minHeight: 0, flexGrow: 1, overflowY: 'auto' }}>
          <Stack direction="row" spacing={1} alignItems="flex-start" sx={{ color: 'text.secondary' }}>
            <UiIcon icon={note.trim() ? 'solar:check-circle-linear' : 'solar:shield-user-linear'} width={19} />
            <Typography variant="body2" sx={{ color: 'inherit' }}>
              {note.trim()
          ...
          {noteError && <Typography variant="body2" sx={{ mt: 2, color: 'error.main' }}>{noteError}</Typography>}
          <TextField
            fullWidth
            multiline
            minRows={12}
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2} sx={{ p: 2.5 }}>
          <Typography variant="caption" sx={{ color: 'text.secondary' }}>{note.length} belgi</Typography>
      ...
      <Dialog open={shareOpen} onClose={() => setShareOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ pb: 3 }}>
          ...
          <TextField
            ...
            slotProps={{ htmlInput: { readOnly: true, 'aria-label': 'Maqolaning doimiy havolasi' } }}
            sx={{ mt: 2.5 }}
          />
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25} sx={{ mt: 2 }}>
            <Button
              ...
              startIcon={<UiIcon icon={shareCopied ? 'solar:check-circle-bold' : 'solar:copy-linear'} width={18} />}
            >
              {shareCopied ? 'Havola nusxalandi' : 'Havolani nusxalash'}
            </Button>
            {canNativeShare && (
              <Button color="inherit" onClick={() => void shareArticle()} startIcon={<UiIcon icon="solar:share-linear" width={18} />}>
                Qurilmada ulashish
              </Button>
            )}
          </Stack>
          ...
          <Stack divider={<Divider flexItem />}>
            {revisions.map((revision) => (
              <Box key={`${revision.version}-${revision.date}`} sx={{ py: 2 }}>
                <Stack direction="row" justifyContent="space-between"><Typography variant="subtitle2">{revision.version}</Typography><Typography variant="caption" sx={{ color: 'text.secondary' }}>{formatUzbekDate(revision.date)}</Typography></Stack>
                <Typography variant="body2" sx={{ mt: 1 }}>{revision.note}</Typography>
                <Typography variant="caption" sx={{ mt: 0.5, display: 'block', color: 'text.secondary' }}>{revision.author}</Typography>
              </Box>
            ))}
          </Stack>
        </DialogContent>
