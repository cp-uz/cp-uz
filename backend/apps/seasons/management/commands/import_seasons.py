...
DEFAULT_DATA_PATH = DATA_DIRECTORY / "season_seed.json"
DEFAULT_SCHEMA_PATH = DATA_DIRECTORY / "season_seed.schema.json"


class Command(BaseCommand):
    help = "Olimpiada mavsumlari, grafigi, resurslari va natijalarini JSON fayldan import qiladi."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help=f"Seed JSON manzili (default: {DEFAULT_DATA_PATH})",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--prune",
            action="store_true",
            help="JSON ichida yo‘q eski route/event/edge va event bolalarini o‘chiradi.",
        )
        parser.add_argument(
            "--clear",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if not path.is_file():
            raise CommandError(f"Seed fayli topilmadi: {path}")
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise CommandError(f"Seed JSON o‘qilmadi: {exc}") from exc
    ...
    def validate_payload_schema(cls, payload: Any):
        try:
            ...
            raise CommandError(f"Season schema o‘qilmadi: {DEFAULT_SCHEMA_PATH}: {exc}") from exc
    ...
    def validate_payload(payload: Any):
        ...
        for season_index, season in enumerate(seasons):
            ...
            for collection in ("routes", "events"):
    ...
    def import_season(self, data: dict[str, Any], *, prune: bool):
        ...
        for event_data in data.get("events", []):
            ...
            self.import_event_children(event, event_data, routes, prune=prune)
    ...
    def import_event(self, season: Season, data: dict[str, Any]) -> Event:
        ...
        return self.upsert(
            ...
            {"season": season, "code": code},
            {
                ...
                "start_date": self.parsed_date(
                    data.get("start_date"), f"event[{code}].start_date"
                ),
        ...
        *,
        ...
        for resource_data in data.get("resources", []):
            ...
            self.upsert(
                ...
                {
                    "type": self.required(
                        resource_data, "type", f"event[{event.code}].resource"
                    ),
                    "title": self.required(
                        resource_data, "title", f"event[{event.code}].resource"
                    ),
        ...
        for source_data in data.get("sources", []):
            ...
            self.upsert(
                ...
                {"event": event, "url": url},
                {
                    ...
                    "title": self.required(
                        source_data, "title", f"event[{event.code}].source"
                    ),
        ...
        for index, result_data in enumerate(data.get("results", [])):
            source_key, team_name = self.import_result(
                event, result_data, index=index, prune=prune
            )
    ...
    def import_participant(self, raw: dict[str, Any] | str) -> Participant:
        ...
        alias = ParticipantAlias.objects.select_related("participant").filter(
        ...
        for index, account_data in enumerate(data.get("platform_accounts", [])):
            ...
            self.upsert(
                ...
                {
                    "url": self.required(
                        account_data, "url", "participant.platform_account"
                    ),
    ...
    def import_team(self, event: Event, data: dict[str, Any], *, prune: bool) -> Team:
        ...
        for index, member_data in enumerate(data.get("members", [])):
            participant = self.import_participant(member_data["participant"])
    ...
    def import_result(
        self, event: Event, data: dict[str, Any], *, index: int, prune: bool
    ) -> tuple[str, str | None]:
        participant = None
        team = None
        team_name = None
        if data.get("participant"):
            participant = self.import_participant(data["participant"])
            subject_key = f"participant:{participant.slug}"
        elif data.get("team"):
            team = self.import_team(event, data["team"], prune=prune)
            team_name = team.name
