...
class EditProposalViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = EditProposal.objects.select_related("article", "submitter").prefetch_related(
            "reviews__reviewer", "status_events__actor"
        )
...
class ReviewHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = ReviewRecord.objects.select_related("article", "proposal", "reviewer")
        if self.request.user.is_staff:
            return queryset
